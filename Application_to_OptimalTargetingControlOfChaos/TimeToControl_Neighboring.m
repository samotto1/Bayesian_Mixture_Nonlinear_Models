...
figure()
p1 = plot(TrainData.X(1,:), TrainData.X(2,:), 'k.', 'MarkerSize', 6);
hold on
p2 = plot(X_RootTraj(1,:), X_RootTraj(2,:), 'gx', 'MarkerSize', 6, ...
    'LineWidth', 1.5);
p3 = plot(x_FP(1), x_FP(2), 'go', 'LineWidth', 3 ,'MarkerSize', 6);
legend([p1,p2,p3], {'Training Data Set', 'Target Path Points', 'Fixed Point'})
hold off
xlabel('x_1')
ylabel('x_2')
title('Poincare Map with Targeting Path Points')
grid on
drawnow
    ...
    for k = 1:N_trials
        ...
        while n_iter <= N_sim_max && interior_count < 10
            ...
            elseif strcmpi(UseTargeting, 'y') % use optimal targeting control
                ...
                can_control = 0;
                l = 0;
                while can_control == 0 && l < Root_Len-1
                    x_root = X_RootTraj(:,Root_Len-1-l);
                    K_root = K_RootTraj(:,:,Root_Len-1-l);
...
%loglogfit = polyfit(log(eps_u_vec([1,2,4,5]))', log(MeanControlTimes([1,2,4,5])), 1);
