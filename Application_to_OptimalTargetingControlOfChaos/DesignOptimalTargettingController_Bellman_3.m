...
% number of unforced root points for value function interpolaton
N_ValFunPts = 500;
...
% Duffing data with no forcing (used as points for value function
% iteration)
...
% Choose root trajectory leading to the nhd of the FP from unforced
% training data
...
% Construct root trajectory points
Root_Len = min(N_ValFunPts-1, idx);
...
N_RandPts = N_ValFunPts - Root_Len - 1;
if N_RandPts > 0
    ValFun_pt_inds = randperm(N_train, N_RandPts);
    X_pts_ValFun = [x_FP, X_RootTraj, X_train(:,ValFun_pt_inds)];
else
    X_pts_ValFun = [x_FP, X_RootTraj];
end
