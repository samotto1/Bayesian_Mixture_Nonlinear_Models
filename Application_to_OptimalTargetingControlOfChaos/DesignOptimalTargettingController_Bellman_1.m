...
eps_u = 0.02;%1e-1; % maximum control action magnitude
...
while MaxValFunChange > tol_ValFun && iter < 100
    ...
    ValFun_pts_new = zeros(1,N_ValFunPts);
    for j=1:N_ValFunPts
        ...
        % update value function
        ValFun_pts_new(j) = 1/2*(x-x_FP)'*Q_x*(x-x_FP) + ...
            1/2*u_star'*R_u*u_star + gamma_ValFun * ...
            ValFun(ith_KernelModel_Eval( ...
            [x;u_star;1], i_star, kernel, KerModels, TrainData ));
        
        fprintf('Updated %d of %d : DeltaV = %.3e \n', j, N_ValFunPts, ...
            abs(ValFun_pts_new(j)-ValFun_pts(j)))
    end
    
    MaxValFunChange = max(ValFun_pts_new-ValFun_pts)
    ValFun_pts = ValFun_pts_new;
