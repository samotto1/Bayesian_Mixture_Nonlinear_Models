...
%   2. Fit linear models to the clusters and estimate error covariances
%   3. Assign points to clusters based on maximum likelihood Gaussian
%   errors
% This algorithm is analogous to K-Means
...
R_Matrices = zeros(2,2,N_LinModels);
Rinv_Matrices = zeros(2,2,N_LinModels);
Rdet_Values = zeros(1,N_LinModels);
...
while any(sqrt(sum(Delta_Centroids.^2,1)) > Tol_centroid) && iter < Iter_Max
    ...
    for i = 1:N_LinModels
        ...
        % determine error covariance (Gaussian error MLE)
        Y_error = DY - M*DX;
        R = 1.0/MdlCounts(i) * (Y_error*Y_error');
        R_Matrices(:,:,i) = R;
        Rinv_Matrices(:,:,i) = pinv(R);
        Rdet_Values(i) = det(R);
    ...
    for j = 1:N_train
        ...
        % Evaluate error likelihoods using each model 
        for ii = 1:N_nearest
            idx_mdl = MdlInds(ii);
            ...
            % evaluate MLE gaussian error distribution from above
            P_error(ii) = 1.0/((2*pi) * sqrt(Rdet_Values(idx_mdl))) *...
                exp(-0.5 * y_error'*Rinv_Matrices(:,:,idx_mdl)*y_error);
            
        end
        
%         % Assign points stochastically
%         boxes = cumsum(P_error)/sum(P_error);
%         r = rand(1);
%         ii_star = find(r<=boxes, 1, 'first');
...
%         MdlIndices(idx_mdl_star, MdlCounts(idx_mdl_star)) = j;
        ...
        % Assign to model based on maximum likelihood
        [~, ii_star] = max(P_error);
        idx_mdl_star = MdlInds(ii_star);
        MdlCounts(idx_mdl_star) = MdlCounts(idx_mdl_star) + 1;
        MdlIndices(idx_mdl_star, MdlCounts(idx_mdl_star)) = j;
