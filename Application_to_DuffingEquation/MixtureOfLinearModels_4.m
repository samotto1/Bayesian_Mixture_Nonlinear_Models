...
%   2. Fit linear models to training data using Gaussians for weighting
...
% minimum allowable singular value of Spatial covariance
...
while any(sqrt(sum(Delta_Centroids.^2,1)) > Tol_centroid) && iter < Iter_Max
    ...
    for i = 1:N_LinModels
        ...
        % Prior proabilities of each model
        Phi_Mdls(i) = MdlCounts(i)/N_train;
        ...
        Dx_pts = X_cluster(1:2,:) - Centroids_x(:,i)*ones(1,MdlCounts(i));
        ...
        w = zeros(1,N_train); % weight of each point
        for j = 1:N_train
            Dx = X_train(1:2,j) - Centroids_x(:,i); % dist from centroid
            w(j) = 1.0/( (2*pi) * sqrt(SigDet_Mdls(i)) ) *...
                exp(-0.5 * Dx'*SigInv_Mdls(:,:,i)*Dx);
        end
         
        % fit linear model
        DX = [X_train(1:2,:) - Centroids_x(:,i)*ones(1,N_train);
            X_train(3:end,:)];
        DY = Y_train - Centroids_y(:,i)*ones(1,N_train);
        
        DXW = DX*diag(sqrt(w)); %DX .* (ones(2+N_bases,1)*sqrt(w));
        DYW = DY*diag(sqrt(w)); %DY .* (ones(2,1)*sqrt(w));
