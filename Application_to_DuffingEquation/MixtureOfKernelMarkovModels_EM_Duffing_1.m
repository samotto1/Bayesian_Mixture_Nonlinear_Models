...
% Gaussian densities
% ** EM algorithm also fits markov transition process among models
% ** Use MLE Gaussians and Markov process as a generative model for
% classification

%% Paramters
N_Models = 20;
N_SubTrain = 300; % number of points to use in Kernel regression
lambda = 0.000; % Ridge regression regularization parameter
Iter_Max = 10;
Tol_centroid = 1e-4;
...
[LinModels] = MixtureOfKernelMarkovModels_EM( TrainData, Params);
...
% Initialize Priors
Phi_hat = zeros(N_Models, N_test+1);
Phi_hat(:,1) = 1/N_Models; % use uniform prior
...
for j = 1:N_test
    ...
    for i = 1:N_Models
        W_Mdls(i) = NormalDist(x, Mu_x(:,i), SigInv_Mdls(:,:,i), ...
            SigDet_Mdls(i)) * T_Mdls(i,:)*Phi_hat(:,j); 
    end
    Phi_hat(:,j+1) = W_Mdls/sum(W_Mdls); % update prior
    
    % Assign to model based on max likelihood
    [~, i_star] = max(Phi_hat(:,j+1));
    ...
    r= rand(1);
    csumPhi = cumsum(Phi_hat(:,j+1));
    i_stoch = find(csumPhi >= r, 1, 'first');
    ...
    for i = 1:N_Models      
        Y_hat_Mix(:,j) =Y_hat_Mix(:,j) + Phi_hat(i,j+1)*...
...
% Initialize Priors
Phi_gen = zeros(N_Models, N_generate+1);
Phi_gen(:,1) = 1/N_Models; % use uniform prior
...
for n = 2:N_generate
    ...
    for i = 1:N_Models
        W_Mdls(i) = NormalDist(x, Mu_x(:,i), SigInv_Mdls(:,:,i), ...
            SigDet_Mdls(i)) * T_Mdls(i,:)*Phi_gen(:,j); 
    end
    Phi_gen(:,j+1) = W_Mdls/sum(W_Mdls); % update prior
    
    % Assign to model based on max likelihood
    [~, i_star] = max(Phi_gen(:,j+1));
    
    % Assign to model based on stochastic choice
    r= rand(1);
    csumPhi = cumsum(Phi_gen(:,j+1));
    i_stoch = find(csumPhi >= r, 1, 'first');
