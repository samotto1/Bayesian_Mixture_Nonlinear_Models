...
% ** use K-means algorithm to fit a mixture of regularized kernel models with
% Gaussian densities
% ** Use MLE Gaussians as a generative model for classification

%% Paramters
N_Models = 40;
TopN = [2,5]; % top models to choose from for each point
...
for j = 1:N_test
    ...
    [~,mdl_inds] = sort(W_Mdls, 'descend');
    W_Renormalized = W_Mdls(mdl_inds(1:TopN(2)))/...
        sum(W_Mdls(mdl_inds(1:TopN(2))));
    for i = 1:TopN(2)      
        Y_hat_Mix(:,j) =Y_hat_Mix(:,j) + W_Renormalized(i)*...
            ( M_matrices(:,:,mdl_inds(i))*K_z + Mu_y(:,mdl_inds(i)) );     
    end
