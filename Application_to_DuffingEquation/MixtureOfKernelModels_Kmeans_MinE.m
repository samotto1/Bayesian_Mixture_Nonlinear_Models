function [LinModels] = MixtureOfKernelModels_Kmeans_MinE( TrainData, Params)
%MixtureOfKernelModels_Kmeans
%Minimum error model assignment
...
while any(sqrt(sum(Delta_Centroids.^2,1)) > Tol_centroid) && iter < Iter_Max
    ...
    for i = 1:N_Models
        ...
        M_matrices(:,indices,i) = DY*W*pinv(K_sub*W + ...
            lambda*eye(MdlCounts(i)));
        
        
        % Find error covariance
        DY_error = DY - M_matrices(:,indices,i)*K_sub;
        R_i = 1.0/MdlCounts(i) * (DY_error*DY_error');
    ...
    MdlCounts_old = MdlCounts;
    MdlIndices_old = MdlIndices;
    MdlCounts = zeros(1,N_Models);
    MdlIndices = ones(N_Models,N_train);
    ...
    for j = 1:N_train
        ...
        for ii = 1:N_nearest
            ...
            ker_indices = MdlIndices_old(idx_mdl,1:MdlCounts_old(idx_mdl));
            y_hat = M_matrices(:,ker_indices,idx_mdl)*K(ker_indices,j) + ...
                Mu_y(:,idx_mdl);
