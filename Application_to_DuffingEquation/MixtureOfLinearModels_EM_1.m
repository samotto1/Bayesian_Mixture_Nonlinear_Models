...
while any(sqrt(sum(Delta_Centroids.^2,1)) > Tol_centroid) && iter < Iter_Max
    ...
    for j = 1:N_train
%         if Sum_P_yxuz(j) < 1e-10 || isinf(Sum_P_yxuz(j)) || isnan(Sum_P_yxuz(j));
%             % assign nearest model     
%             x = X_train(1:2,j);
%             % Evaluate likelihood each model
%             P_Mdls = zeros(1,N_LinModels);
%             for i = 1:N_LinModels    
%                 P_Mdls(i) = NormalDist(x, Mu_x(:,i), ...
%                     1/sig_init*eye(2), sig_init^2);
%             end   
% %             % Assign to model based on max likelihood
% %             [~, i_star] = max(P_Mdls);
% %             Q_bar(j,:) = zeros(1,N_LinModels);
% %             Q_bar(j,i_star) = 1.0;
%             
%             Q_bar(j,:) = P_Mdls/sum(P_Mdls);
% 
%             %Q_bar(j,:) = ones(1,N_LinModels)/N_LinModels;
%         else
...
%     Y_hat(:,j) = zeros(2,1);
%     for i = 1:N_LinModels
%         Y_hat(:,j) = Y_hat(:,j) + P_Mdls(i)*( A_matrices(:,:,i)*...
%             (x - Mu_x(:,i)) + B_matrices(:,:,i)*p +...
%             Mu_y(:,i) );
%     end
end
